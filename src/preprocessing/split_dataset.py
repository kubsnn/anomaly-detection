...
def setup_dataset_split(base_path: str,dir: str, train_ratio=0.7, val_ratio=0.2, test_ratio=0.1):
    logger = setup_logging(__name__)

    if not abs(train_ratio + val_ratio + test_ratio - 1.0) < 1e-9:
        raise ValueError("Ratios must sum to 1")

    normal_dir = Path(base_path) / 'videos' / dir
...
if __name__ == "__main__":
    ...
    try:
        setup_dataset_split(base_path, 'normal')
        setup_dataset_split(base_path, 'fight')
